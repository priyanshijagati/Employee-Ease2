...
const Register = () => {
    ...
    const handleRegister = e => {
        ...
        if (password.length < 6) { 
            ...
            toast("Enter email in correct format");
            return;
        }
        // else if (!email.value.length || !password.value.length || !name.value.length || !accountNo.value.length || !salary.value.length || !designation.value.length) {
        //     toast("Fill all the fields");
        //     return;
        // }
        else if (!/^\d{8}$/.test(accountNo)) {
            toast("Enter account number in correct format");
            return;
        }
        else if (!Number(salary)) {
            toast("Enter salary in correct format");
            return;
        }
    ...
    return (
        <div className="grid lg:grid-cols-5 grid-cols-2">
            ...
            <div className="flex justify-center items-center col-span-3">
                <img className="w-3/4" src="https://i.ibb.co/18FkvCn/pngwing-com.png" alt="" />
            </div>
        </div>
    );
};
